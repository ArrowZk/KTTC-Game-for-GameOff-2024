...
func _ready() -> void:
	...
	GLOBAL.can_run = true
	GLOBAL.can_move_boxes = true
	GLOBAL.can_throw_stuff = true
	GLOBAL.can_crouch = true
...
func setup_audio_loop():
	var audio_stream_player = $Extras/AudioStreamPlayer
	
	# Configuración del stream
	audio_stream_player.stream.loop = true
	audio_stream_player.stream.loop_begin = 17.0 * 1000  # Milisegundos
	
	# Reproducir
	audio_stream_player.play()

func _process(delta: float) -> void:
	if is_instance_valid(get_node("NPCs/Agent")) and npc_not_killed:
		pass
	elif npc_not_killed:
		npc_not_killed=false
		balloon.start(dialogue_resource,"outro")
